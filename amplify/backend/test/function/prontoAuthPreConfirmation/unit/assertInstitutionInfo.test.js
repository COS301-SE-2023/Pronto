...
} = require('../../../../function/prontoAuthPreConfirmation/src/assertInstitutionInfo');
...
describe('Input Validation and Error handling', () => {
  test(`should throw "Invalid Institution Id: InstitutionId =  null"`, async () => {
    ...
    await expect(getAndSetInstitutionDetails(nullInstitutionId)).rejects.toThrow(
      /^Invalid Institution Id: InstitutionId =/,
    );
  });
  test(`should throw "Failed To retrieve institution details"`, async () => {
    const someInstitutionId = 'someInstitutionId';

    await expect(getAndSetInstitutionDetails(someInstitutionId)).rejects.toThrow(
  ...
  test(`should throw "Failed to retrieve list for the institution"`, async () => {
    ...
    await expect(getLectureEmailsFromInstitution(someInstitutionId)).rejects.toThrow(
      /^Failed to retrieve list for the institution/,
    );
  });
  ...
  test(`should throw "Invalid email"`, async () => {
    await expect(isLectureEmailPartOfInstitution(null)).rejects.toThrow(/^Invalid email/);
  });
  ...
  test(`should throw "Failed to retrieve admin for the institution...Failed To retrieve institution details" on isAdminAllocated`, async () => {
    const someInstitutionId = 'someInstitutionId';
    await expect(isAdminAllocated(someInstitutionId)).rejects.toThrow(/Failed to retrieve admin for the institution/);
