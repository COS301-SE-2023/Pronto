...
const preAuth = require('../../../../function/prontoAuthPreConfirmation/src/index');

jest.mock('../../../../function/prontoAuthPreConfirmation/src/assertInstitutionInfo', () => ({
...
describe('Input Validation and Error handling', () => {
  test(`should throw "User role not provided on ClientMetadata"`, async () => {
    ...
    await expect(preAuth.handler(eventWithNullRole)).rejects.toThrow(/^User role not provided on ClientMetadata$/);
  });
  test('should throw "Invalid User Role"', async () => {
    ...
    await expect(preAuth.handler(studentEventWithAdminInvalidRole)).rejects.toThrow(/Invalid User Role/);
  });
  test('should throw "Unrecognised user pool app client ID"', async () => {
    ...
    await expect(preAuth.handler(studentEventWithAdminInvalidClientId)).rejects.toThrow(
  ...
  test('should throw "Institution has an admin already. institutionId"', async () => {
    await expect(preAuth.handler(adminEvent)).rejects.toThrow(/Institution has an admin already. institutionId/);
  });
  test('should throw "Lecturer email is not part of the Institution"', async () => {
    await expect(preAuth.handler(lecturerEvent)).rejects.toThrow(/Lecturer email is not part of the Institution/);
  });
});
